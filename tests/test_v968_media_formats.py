#!/usr/bin/env python3
"""v968 -- the media extension law lives in ONE place, and knows .avif.
...
against the source before answering:
  ...
  * The reason a format could be half-supported at all is the real defect: the
    extension tuples existed TWICE, in ph_media_loader and in uls_routes, with
...
MUTATION PROBE (run 20.09.2026, each mutation applied and reverted):
  ...
  4. literal tuple restored in uls_routes              -> L2 caught
...
def main():
    print("v968: one extension law, and it knows AVIF")
    ...
    check(not media_literals(routes_c),
          "L2 uls_routes carries no literal MEDIA extension tuple of its own")
    ...
    check("__version__" not in pbody,
          "L6 the probe never guesses from a version string -- v965's lesson")
...
if __name__ == "__main__":
    rc = main()
    print("v968 media formats: %s" % ("FAIL (%d)" % len(failures) if failures else "PASS"))
    sys.exit(rc)
